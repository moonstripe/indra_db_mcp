#!/usr/bin/env bun
/**
 * Batch trial runner with version tracking
 */
...
async function runTrial(prompt: string, trialId: string): Promise<TrialResult> {
  ...
  return { prompt, searched, remembered, remember_content, time_ms: Date.now() - start };
}
...
async function main() {
  ...
  console.log(`🧪 Batch Trial Runner (${VERSION})`);
  ...
  for (let i = 0; i < TEST_PROMPTS.length; i++) {
    ...
    for (let t = 0; t < numTrials; t++) {
      const trialId = `batch_${VERSION}_${i}_${t}_${Date.now()}`;
  ...
  const filename = `results_${VERSION.replace(/\./g, '')}_${Date.now()}.json`;
